...
<template>
  <v-navigation-drawer v-model="ndwDefault" location="right" mobile>
    <v-list-item title="Projects" to="/Projects" />
    <v-list-item title="Tasks" to="/Tasks" />
    <template #append>
      <v-menu>
        <template #activator="{ props }">
          <v-btn append-icon="mdi-account-circle" :text="authStore.name" v-bind="props" block />
        </template>
        <v-list>
